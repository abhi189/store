...
if [[ "$1" = "$ENV_QA" ]]; then
  ...
  docker tag sites:v$2 181564704724.dkr.ecr.us-west-2.amazonaws.com/qa/sites:latest
  docker tag sites:v$2 181564704724.dkr.ecr.us-west-2.amazonaws.com/qa/sites:v$2
  info "$(docker images | grep $2 | grep amazonaws | grep sites | grep qa)"
  info "$(docker images | grep latest | grep amazonaws | grep sites | grep qa)"
fi

if [ "$1" = "$ENV_PROD" ]; then
  debug "Tag versions for PROD AWS ECR"
  docker tag sites:v$2 181564704724.dkr.ecr.us-west-2.amazonaws.com/prod/sites:latest
  docker tag sites:v$2 181564704724.dkr.ecr.us-west-2.amazonaws.com/prod/sites:v$2
  info "$(docker images | grep $2 | grep amazonaws | grep sites | grep prod)"
  info "$(docker images | grep latest | grep amazonaws | grep sites | grep prod)"
fi
...
if [[ "$1" = "$ENV_QA" ]]; then
  ...
  docker push 181564704724.dkr.ecr.us-west-2.amazonaws.com/qa/sites:latest
  docker push 181564704724.dkr.ecr.us-west-2.amazonaws.com/qa/sites:v$2
  AWS_ECR_DOCKER_IMG_DIGEST=$(aws ecr describe-images --repository-name qa/sites --query 'sort_by(imageDetails,& imagePushedAt)[-1].imageDigest' | tr -d '"')
fi

if [ "$1" = "$ENV_PROD" ]; then
  debug "Pushing docker images to PROD AWS ECR"
  docker push 181564704724.dkr.ecr.us-west-2.amazonaws.com/prod/sites:latest
  docker push 181564704724.dkr.ecr.us-west-2.amazonaws.com/prod/sites:v$2
  AWS_ECR_DOCKER_IMG_DIGEST=$(aws ecr describe-images --repository-name prod/sites --query 'sort_by(imageDetails,& imagePushedAt)[-1].imageDigest' | tr -d '"')
fi

DOCKER_IMG_DIGEST=$(docker inspect --format='{{index .RepoDigests 0}}' sites:v$2 | cut -d'@' -f2)
...
debug "Updating current version of sites app"

if [[ "$1" = "$ENV_QA" ]]; then
  debug "Starting sites app on QA kubernetes cluster"
  kubectl apply -f ../minikube/k8s-qa/sites
fi

if [ "$1" = "$ENV_PROD" ]; then
  debug "Starting sites app on PROD kubernetes cluster"
  kubectl apply -f ../minikube/k8s-prod/sites
fi
...
kubectl set env deployment/sites DEPLOY_DATE="$(date)"

debug "Sites pods:"
kubectl get pods | grep sites | grep -v web

success "Deploy finished, check app logs with $(highlight 'kubectl logs -f <sites-hash>')"
